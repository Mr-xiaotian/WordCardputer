...
void drawScoreSelect()
{
    if (scoreLevel == 0) {
        ...
        for (int s = 1; s <= 5; s++) {
            items.push_back(String("Level ") + s);
        }
...
void loopScoreSelectMode()
{
    if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed())
    {
        ...
        if (st.enter)
        {
            ...
            vocabLabel = "score:" + String(selectedScore) + "/" + String(groupListIndex);
            appMode = MODE_STUDY;
            initStudyMode();
        }
    }
