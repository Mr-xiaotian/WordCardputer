...
enum AppMode {
    ...
    MODE_CLASSIFY_SELECT, // 分类方式选择模式
    MODE_FILE_SELECT,     // 文件选择（按词源）模式
    MODE_SCORE_SELECT,    // 文件选择（按 Score）模式
    MODE_STUDY,        // 学习模式
...
String vocabLabel = "";      // 当前加载词库的来源标签，如 "chapter:demo/01" 或 "score:3/0"

std::vector<String> langItems = {"日语", "英语"};  // 语言选择菜单项
int langIndex = 0;                           // 语言选择菜单索引

std::vector<String> classifyItems = {"按词源分类", "按Score分类"};  // 分类方式菜单项
int classifyIndex = 0;                               // 分类方式菜单索引
...
// 该模式拥有专属的 selectedSource / selectedChapter 变量
