...
def generate_tts(
    ...
    参数:
        api_key (str): 你的 MiniMax API 密钥
        text (str): 要转换的文本
        output_path (str): 输出文件路径 (默认 'output.wav')
        model (str): 使用的模型 (默认 speech-2.6-hd)
        voice_id (str): 声音ID (默认 Japanese_Whisper_Belle)
        language (str): 语言增强设置 (默认 Japanese)
        sample_rate (int): 采样率 (默认 32000)
        bitrate (int): 比特率 (默认 128000)
        pitch (int): 音高调整 (范围 -12 ~ 12)
        speed (float): 语速 (0.5 ~ 2.0)
        volume (float): 音量 (0.1 ~ 10)
        emotion (str): 语音情感，可选 [happy, calm, sad, angry...]
    返回:
        bool: 是否生成成功
        str: 输出文件路径或错误信息
    """
